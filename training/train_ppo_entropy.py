#!/usr/bin/env python3
"""
Training script for PPO with Entropy Weighting on MuJoCo Hopper.

This script provides a complete training pipeline for PPO with entropy weighting
on the Hopper environment, including configuration loading, training, evaluation,
and model saving.
"""
...
def create_algorithm(env: gym.Env, config: dict) -> PPOEntropy:
    """Create PPO with entropy weighting algorithm with specified configuration."""
    ...
    algorithm = PPOEntropy(
        ...
        target_entropy=algo_config.get('target_entropy', None),
        ent_coef_lr=algo_config.get('ent_coef_lr', 0.001),
        ent_coef_decay=algo_config.get('ent_coef_decay', 0.999),
        min_ent_coef=algo_config.get('min_ent_coef', 0.001),
        max_ent_coef=algo_config.get('max_ent_coef', 1.0),
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train PPO with Entropy Weighting on MuJoCo Hopper')
    parser.add_argument('--config', type=str, default='configs/ppo_entropy_config.yaml',
    ...
    parser.add_argument('--log_dir', type=str, default='logs/ppo_entropy',
                       help='Directory to save logs')
    parser.add_argument('--model_dir', type=str, default='models/ppo_entropy',
                       help='Directory to save models')
    parser.add_argument('--render', action='store_true',
                       help='Render during evaluation')
    parser.add_argument('--verbose', type=int, default=1,
                       help='Verbosity level')
    parser.add_argument('--plot_entropy', action='store_true',
                       help='Plot entropy adaptation curves')
    ...
    try:
        algorithm.learn(
            ...
            tb_log_name="PPO_Entropy_Hopper",
            reset_num_timesteps=True,
            progress_bar=True
        )
        
        # Save final model
        final_model_path = model_dir / "ppo_entropy_hopper_final.pkl"
        ...
        # Plot entropy adaptation if requested
        if args.plot_entropy:
            print("\nPlotting entropy adaptation curves...")
            entropy_plot_path = log_dir / "entropy_adaptation.png"
            algorithm.plot_entropy_adaptation(str(entropy_plot_path))
        ...
        # Save entropy adaptation data
        ent_coefs, entropies = algorithm.get_entropy_coefficient_history()
        entropy_data = {
            "entropy_coefficients": ent_coefs.tolist(),
            "entropies": entropies.tolist(),
            "target_entropy": algorithm.target_entropy
        }
        entropy_path = log_dir / "entropy_adaptation_data.json"
        ...
        print(f"Final entropy coefficient: {algorithm.ent_coef_tensor.item():.4f}")
        
    except KeyboardInterrupt:
        print("\nTraining interrupted by user")
        
        # Save current model
        interrupted_model_path = model_dir / "ppo_entropy_hopper_interrupted.pkl"
